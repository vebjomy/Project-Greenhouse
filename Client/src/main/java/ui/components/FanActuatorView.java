...
public class FanActuatorView {
  ...
  public static Pane create(String name, String state, Consumer<Boolean> onToggle) {
    ...
    try {
      ...
      fanIcon.setFitHeight(ICON_SIZE);
    ...
    // === 1. Determine Status Color, Text, and Animation ===
    Color baseColor;
    Color darkColor;
    ...
    if (isOn) {
      baseColor = Color.web("#4CAF50"); // Green - Active
      darkColor = Color.web("#388E3C"); // Dark Green
      activeIndicatorColor = Color.web("#4CAF50");
      statusText = "RUNNING";
      if (rotation != null) {
    ...
    onButton.setOnAction(e -> {
      ...
      // Visual update logic remains here
      if (finalRotation != null) {
        finalRotation.play();
      }
      // Note: Immediate visual feedback for the button handler (retained logic)
      mainStatusLabel.setText("ON");
      mainStatusLabel.setTextFill(Color.web("#388E3C"));
    });
    ...
    if (fanIcon != null) {
      iconBox.getChildren().add(fanIcon);
    }
