    ...
    return api.getTopology()
            .thenApply(topology -> {
              ...
              for (Topology.Node node : topology.nodes) {
                ...
                result.append("   Sensors: ").append(node.sensors != null ? node.sensors.size() : 0).append("\n");
                result.append("   Actuators: ").append(node.actuators != null ? node.actuators.size() : 0).append("\n");
                result.append("\n");
              }
  ...
  private CompletableFuture<String> handleStatus() {
    ...
    return CompletableFuture.completedFuture(
            "📊 Status:\n" +
                    ...
                    "Connected: " + (mainApp.isConnected() ? "Yes ✅" : "No ❌") + "\n" +
                    "Nodes in cache: " + nodeCount
    );
  }
